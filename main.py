...
# --- Конфигурация ролей, баланса и сообщений ---
...
ROLE_BALANCE = {
    ...
    12: ["Босс Мафии", "Киллер Мафии", "Подручный Мафии", "Босс Якудзы", "Ниндзя", "Маньяк", "Шериф", "Сержант", "Доктор", "Судья", "Куртизанка", "Журналист"],
    15: ["Босс Мафии", "Киллер Мафии", "Подручный Мафии", "Босс Якудзы", "Ниндзя", "Подручный Якудзы", "Маньяк", "Ведьма", "Шериф", "Сержант", "Доктор", "Судья", "Куртизанка", "Журналист", "Тюремщик"],
    21: ["Босс Мафии", "Киллер Мафии", "Подручный Мафии", "Босс Якудзы", "Ниндзя", "Подручный Якудзы", "Маньяк", "Путана", "Ведьма", "Шериф", "Сержант", "Куртизанка", "Доктор", "Журналист", "Бомж", "Почтальон", "Тюремщик", "Стрелок", "Амур", "Судья", "Ветеран"],
    30: [
...
NIGHT_ACTION_MESSAGES = {
    "Шериф": "Шериф вышел на ночную проверку... 👮‍♂️",
    "Доктор": "Доктор спешит на помощь... 🩺",
    "Куртизанка": "Кто-то этой ночью не сможет уснуть в одиночестве... 💋",
    "Тюремщик": "Тюремщик выводит заключенного в камеру... ⛓️",
    "Журналист": "Журналист ищет связи между игроками... 📝",
    "Бомж": "Бомж притаился в тени, наблюдая за одним из домов... 👁️",
    "Почтальон": "Ночной почтальон доставляет таинственное письмо... ✉️",
    "Ветеран": "Ветеран решил быть начеку этой ночью... 🎖️",
    "Ведьма": "Ведьма начала свой темный ритуал... 🔮",
    "Амур": "Амур натягивает тетиву своего лука... ❤️",
    "Босс Мафии": "Мафия сделала свой выбор... 🔪",
    "Киллер Мафии": "Киллер Мафии готовит свой особый выстрел... 🎯",
    "Босс Якудзы": "Якудза выносит приговор... 🗡️",
    "Маньяк": "На улицах города снова слышны шаги одинокого убийцы... 🩸"
}
...
def get_game_by_user(user_id):
    return next((g for g in games.values() if user_id in g.players), None)
...
class Game:
    ...
    def process_night(self):
        ...
        for player_id, action in night_actions.get('Почтальон', {}).items():
            ...
            if not sender or sender.is_blocked or sender.is_jailed:
                continue
            msg_sender = self.get_player(int(action['target'])) if str(action['target']).isdigit() else None
            receiver = self.get_player(int(action['target2'])) if str(action['target2']).isdigit() else None
        ...
        for player_id, action in night_actions.get('Журналист', {}).items():
            jp = self.get_player(player_id)
            if not jp or jp.is_blocked or jp.is_jailed:
                continue
            p1 = self.get_player(int(action['target'])) if str(action['target']).isdigit() else None
            p2 = self.get_player(int(action['target2'])) if str(action['target2']).isdigit() else None
            if not p1 or not p2:
                continue
        ...
        for player_id, action in night_actions.get('Бомж', {}).items():
            bum = self.get_player(player_id)
            if not bum or bum.is_blocked or bum.is_jailed:
                continue
            target = self.get_player(int(action['target'])) if str(action['target']).isdigit() else None
            if not target:
                continue
    ...
    def resolve_vote(self):
        ...
        voters_by_target = defaultdict(list)

        for player in self.players.values():
            if player.is_alive and player.voted_for is not None:
                vote_counts[player.voted_for] += 1
                voters_by_target[player.voted_for].append(player.tag)
        ...
        for pid, count in vote_counts.items():
            target_player = self.get_player(pid)
            if target_player:
                result_text += f"- За {target_player.tag}: {count} голос(а) ({', '.join(voters_by_target[pid])})\n"
        ...
        if not candidate or not candidate.is_alive:
            bot.send_message(self.chat_id, "Ошибка: цель голосования не найдена или уже мертва.")
            return
...
def start_game(message):
    ...
    roles_in_game = sorted([p.role for p in game.players.values()])
    player_tags = [p.tag for p in game.players.values()]

    start_message = (
        "🔥 **Игра началась!** Роли розданы!\n\n"
        f"**Всего игроков:** {len(game.players)}\n"
        f"**Участники:** {', '.join(player_tags)}\n\n"
        f"**Роли в этой игре:**\n- " + "\n- ".join(roles_in_game) +
        "\n\nНачинается первая ночь. Проверьте личные сообщения от бота, чтобы сделать ход."
    )
    bot.send_message(game.chat_id, start_message, parse_mode="Markdown")
...
def start_night(message):
    ...
    bot.send_message(game.chat_id, f"🌙 Наступила ночь №{game.day}. Город засыпает... Игроки делают ходы в личных сообщениях.")
...
def send_action_keyboard(player, game):
    ...
    if role in ['Шериф', 'Доктор', 'Куртизанка', 'Тюремщик', 'Маньяк', 'Бомж', 'Ведьма', 'Босс Мафии', 'Босс Якудзы', 'Киллер Мафии']:
...
def handle_action_callback(call):
    ...
    if len(parts) < 3:
        bot.answer_callback_query(call.id, "Некорректный ход.")
        return
    ...
    try:
        ...
        print(f"Ошибка в callback хода: {e}")
        bot.answer_callback_query(call.id, "Ошибка обработки хода.")
        return

    if is_final_action:
        player.night_action_done = True
        role_to_notify = player.role
        if player.role == 'Ветеран' and player.night_target != 'alert':
            role_to_notify = None
...
def start_vote(message):
    ...
    bot.send_message(
        game.chat_id,
        f"☀️ Наступил день! Время для голосования.\n"
...
def handle_vote_callback(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, "Некорректная цель голосования.")
        return
